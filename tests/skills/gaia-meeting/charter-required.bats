...
@test "missing --charter exits BLOCKED with an actionable error" {
  [ -x "$HELPER" ]
  run "$HELPER"
  [ "$status" -ne 0 ]
  [ "$status" -ne 127 ]
...
@test "missing --charter writes nothing to the state file" {
  [ -x "$HELPER" ]
  run "$HELPER"
  [ ! -f "$MEETING_STATE_FILE" ]
}

@test "inline --charter records the charter in state" {
...
@test "an empty --charter is treated as missing -> BLOCKED" {
  [ -x "$HELPER" ]
  run "$HELPER" --charter ""
  [ "$status" -ne 0 ]
  [ "$status" -ne 127 ]
