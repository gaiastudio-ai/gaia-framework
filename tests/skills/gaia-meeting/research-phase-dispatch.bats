...
@test "--print-allowlist with web enabled emits exactly Read, Grep, Glob, Bash, WebSearch, WebFetch" {
  run "$HELPER" --print-allowlist
  [ "$status" -eq 0 ]
  [ "$output" = "Read,Grep,Glob,Bash,WebSearch,WebFetch" ]
}

@test "--print-allowlist --no-web emits exactly Read, Grep, Glob, Bash" {
  run "$HELPER" --print-allowlist --no-web
  [ "$status" -eq 0 ]
  [ "$output" = "Read,Grep,Glob,Bash" ]
}

@test "allowlist never contains write-capable tools when web is on" {
  run "$HELPER" --print-allowlist
  [ "$status" -eq 0 ]
  [[ "$output" != *"Write"* ]]
  [[ "$output" != *"Edit"* ]]
...
@test "allowlist never contains write-capable tools under --no-web" {
  run "$HELPER" --print-allowlist --no-web
  [ "$status" -eq 0 ]
  [[ "$output" != *"Write"* ]]
  [[ "$output" != *"Edit"* ]]
...
@test "--check-web-flag returns enabled by default" {
  run "$HELPER" --check-web-flag
  [ "$status" -eq 0 ]
  [ "$output" = "enabled" ]
}

@test "--check-web-flag returns disabled when --no-web is set" {
...
@test "--check-research-flag returns enabled by default" {
  run "$HELPER" --check-research-flag
  [ "$status" -eq 0 ]
  [ "$output" = "enabled" ]
}

@test "--check-research-flag returns skipped when --skip-research is set" {
  run "$HELPER" --check-research-flag --skip-research
  [ "$status" -eq 0 ]
  [ "$output" = "skipped" ]
}

@test "--emit-frontmatter --skip-research records the research phase as skipped" {
  run "$HELPER" --emit-frontmatter --skip-research
  [ "$status" -eq 0 ]
  [[ "$output" == *"research_phase: skipped"* ]]
}

@test "--emit-frontmatter --no-web records web search as disabled" {
  run "$HELPER" --emit-frontmatter --no-web
  [ "$status" -eq 0 ]
  [[ "$output" == *"web_search: disabled"* ]]
}

@test "--emit-frontmatter with web on records web search as enabled" {
...
@test "--skip-research and --no-web are both reflected in the frontmatter" {
