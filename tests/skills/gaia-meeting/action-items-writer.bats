...
@test "header gains schema_version: 2 on first v2 write to a v1 registry" {
  _seed_v1_registry
  draft="$(_seed_draft 1)"
...
@test "pre-existing v1 entries remain byte-identical after a v2 append" {
  _seed_v1_registry
...
@test "v2 entry carries all required fields (id, created, source_meeting, type, priority, status, target_command, assignee, context_for_target, acceptance)" {
  _seed_v1_registry
  draft="$(_seed_draft 1)"
  run "$WRITER" --registry "$REGISTRY" --drafts "$draft" --source-meeting "meeting-2026-05-07-fixture" --date 2026-05-07
  [ "$status" -eq 0 ]
...
@test "generated ids follow the dated sequence format" {
  _seed_v1_registry
  draft="$(_seed_draft 1)"
  run "$WRITER" --registry "$REGISTRY" --drafts "$draft" --source-meeting "meeting-2026-05-07-fixture" --date 2026-05-07
  [ "$status" -eq 0 ]
...
@test "header bump is idempotent — a second run does not duplicate schema_version" {
  _seed_v1_registry
  draft="$(_seed_draft 1)"
...
@test "daily counter — a second meeting the same day continues the count, the next day restarts at 1" {
  _seed_v1_registry
  d1="$(_seed_draft 3)"
...
@test "discussion-only items emit type: discussion-only and target_command: 'no target — discussion-only'" {
  _seed_v1_registry
...
@test "writer rejects an unknown type rather than silently coercing it" {
  _seed_v1_registry
...
@test "writer creates the registry from scratch when the --registry path is missing" {
  rm -f "$REGISTRY"
  draft="$(_seed_draft 1)"
