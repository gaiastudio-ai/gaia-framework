...
@test "AF-22-3 Bug-1: gaia-init prelude uses sed (no \$N token vulnerable to Claude Code interpolation)" {
...
@test "AF-22-3 Bug-1: gaia-create-prd prelude uses sed (no \$N)" {
  grep -qF "sed -n 's/^SURFACE-WARNING: //p'" "$PLUGIN_ROOT/skills/gaia-create-prd/SKILL.md"
}

@test "AF-22-3 Bug-1: framework-wide — zero SKILL.md files still use the broken awk pattern (any variant)" {
...
@test "AF-22-3 Bug-1: sed extraction produces the bare path (no SURFACE-WARNING: prefix)" {
  local result
...
@test "AF-22-3 Bug-2: gaia-init SKILL.md references canonical .gaia/config/project-config.yaml" {
...
@test "AF-22-3 Bug-2: generate-config.sh writes to .gaia/config/ (canonical agreement)" {
...
@test "AF-22-3 Bug-3: prd-template H2s use numeric outline prefix (## 1. Overview, etc.)" {
...
@test "AF-22-3 Bug-5: gaia-create-ux finalize SV-01 uses resolved-path display (not hard-coded docs/)" {
...
@test "AF-22-3 minor: zero SKILL.md files use the bare relative !scripts/write-checkpoint.sh path" {
...
@test "AF-22-3 minor: checkpoint dispatches now use \${CLAUDE_PLUGIN_ROOT}/scripts/write-checkpoint.sh" {
