...
# TC-CFGB-1
@test "platform-edit.sh add (no arg) emits baseline menu + exits 0" {
...
@test "platform-edit.sh add (no arg) mentions the kebab-case extensibility regex" {
  run --separate-stderr "$PLATFORM_EDIT" --config "$CFG" add
  [ "$status" -eq 0 ]
  [[ "$stderr" == *"[a-z][a-z0-9-]*"* ]] || [[ "$stderr" == *"kebab"* ]]
}

@test "platform-edit.sh add with valid id still works (regression)" {
  run "$PLATFORM_EDIT" --config "$CFG" add ios
  [ "$status" -eq 0 ]
  grep -qE '^platforms:' "$CFG"
}

@test "platform-edit.sh add with invalid id still rejects with exit 1 (regression)" {
...
# TC-CFGB-2
@test "gaia-config-show SKILL.md documents the TOC default" {
  grep -qE 'TOC|toc|top-level section list' "$SKILLS/gaia-config-show/SKILL.md"
}

@test "gaia-config-show SKILL.md documents --full for byte-verbatim render" {
  grep -qE '\-\-full' "$SKILLS/gaia-config-show/SKILL.md"
}

@test "gaia-config-show SKILL.md describes no-arg = TOC (default), <section> = single, --full = byte-verbatim" {
...
# TC-CFGB-3
@test "gaia-config-tool SKILL.md scaffold uses tools: heading + category comment block" {
...
@test "gaia-config-tool scaffold no longer hardcodes 3 provider entries" {
...
# TC-CFGB-4
@test "compliance is NOT in schema required (absent is permitted)" {
...
@test "compliance section regimes minItems requirement is relaxed or marked optional" {
...
@test "gaia-config-compliance SKILL.md documents scaffold-skip option (semantic default)" {
...
# TC-CFGB-5
@test "gaia-config-tool SKILL.md documents orphan-rejection for unknown adapter categories" {
  grep -qE 'orphan.rejection|unknown.{0,10}category|not a known adapter' \
    "$SKILLS/gaia-config-tool/SKILL.md"
}

@test "gaia-config-tool error message points users to /gaia-list-tools" {
  grep -qE '/gaia-list-tools|list-adapters' "$SKILLS/gaia-config-tool/SKILL.md"
}

@test "orphan-rejection pattern documented as a config-skill author convention" {
...
@test "existing config with explicit empty regimes still acceptable per schema" {
