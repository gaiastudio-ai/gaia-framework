...
@test "check-monolith-shard-sync.sh accepts a positional project-root" {
  run bash "$PLUGIN_ROOT/scripts/check-monolith-shard-sync.sh" "$BATS_TEST_TMPDIR"
  [ "$status" -eq 0 ]
}

@test "unknown --flag still exits 64 (positional alias did not weaken flag validation)" {
  run bash "$PLUGIN_ROOT/scripts/check-monolith-shard-sync.sh" --bogus-flag
  [ "$status" -eq 64 ]
}

@test "root still works (no regression)" {
...
@test "no skill-local script is referenced via the bare !scripts/ prefix" {
...
@test "each skill-local script is referenced via CLAUDE_PLUGIN_ROOT/skills/.../scripts/" {
  local skill="$PLUGIN_ROOT/skills/gaia-create-story/SKILL.md"
  for s in slugify generate-frontmatter scaffold-story validate-canonical-filename \
           validate-frontmatter append-edge-case-acs append-edge-case-tests; do
...
@test "every referenced skill-local script actually exists on disk" {
  for s in slugify generate-frontmatter scaffold-story validate-canonical-filename \
           validate-frontmatter append-edge-case-acs append-edge-case-tests; do
...
@test "per-epic-shard-absent log is a non-alarming info note" {
  run grep -F 'has no optional per-epic shard' "$PLUGIN_ROOT/scripts/transition-story-status.sh"
  [ "$status" -eq 0 ]
}

@test "the old misleading 'no per-epic shard entry found' wording is gone" {
...
@test "test-strategy finalize calls checkpoint.sh write with --step" {
  run grep -E '"\$CHECKPOINT" write --workflow "\$WORKFLOW_NAME" --step' \
    "$PLUGIN_ROOT/skills/gaia-test-strategy/scripts/finalize.sh"
  [ "$status" -eq 0 ]
}

@test "test-strategy finalize uses --type (no emit subcommand, no --event/--status)" {
...
@test "the corrected lifecycle-event call shape exits 0 against the real script" {
...
@test "retro finalize sentinel marker uses retrospective.yaml" {
  run grep -F 'retrospective.yaml' "$PLUGIN_ROOT/skills/gaia-retro/scripts/finalize.sh"
  [ "$status" -eq 0 ]
}

@test "the dead retrospective.json marker is gone" {
...
@test "project_kind=application emits no warning on resolve" {
...
@test "a genuinely non-canonical project_kind emits the reworded note (not 'unknown')" {
  mkdir -p "$BATS_TEST_TMPDIR/.gaia/config"
