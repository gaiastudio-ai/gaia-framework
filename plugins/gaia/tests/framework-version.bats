...
@test "source + call outputs valid semver + exit 0" {
  [ -f "$LIB_SRC" ]
...
@test "output matches the version field in plugin.json (plugin.json fallback path)" {
...
@test "missing plugin.json returns exit 1, stderr diagnostic" {
  cp "$LIB_SRC" "$STAGED_LIB"
  run --separate-stderr bash -c "PATH=/usr/bin:/bin source '$STAGED_LIB' && resolve_framework_version"
  [ "$status" -eq 1 ]
...
@test "empty version field returns exit 2" {
  cp "$LIB_SRC" "$STAGED_LIB"
...
@test "absent version key returns exit 2" {
  cp "$LIB_SRC" "$STAGED_LIB"
  cat > "$STAGED_MANIFEST" <<JSON
{
  "name": "gaia"
...
@test "source guard prevents double-source side effects" {
  run bash -c "
    source '$LIB_SRC'
...
@test "GAIA_FRAMEWORK_VERSION is exported after successful resolve" {
  run bash -c "
    source '$LIB_SRC'
...
@test "trust-boundary doc comment block present" {
  grep -qE "(trust boundary|SR-60|plugin distribution)" "$LIB_SRC"
}

@test "template-header.sh does NOT define resolve_framework_version" {
...
@test "framework-version.sh defines resolve_framework_version" {
