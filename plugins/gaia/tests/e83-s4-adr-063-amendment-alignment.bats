...
@test "row at monolith L126 == shard 02-2 L66 (bytewise)" {
...
@test "detail block bytewise-identical between monolith and shard" {
...
@test "§19.4 section header present in traceability-matrix.md" {
  run grep -F '## §19.4 AF-2026-05-09-5 / E83' "$TRACE"
  [ "$status" -eq 0 ]
}

@test "§19.4 maps amendment to .." {
...
@test "§19.4 lists all six E83 stories" {
...
@test "§19.4 maps action items /11/12" {
...
@test "§19.4 maps three user memory rules" {
...
@test "§19.4.1 Coverage Notes header present" {
  run grep -F '### 19.4.1 Coverage Notes' "$TRACE"
  [ "$status" -eq 0 ]
}

@test "§19.4.1 documents MUST-PASS gates" {
...
@test "§19.4.1 documents strict-sequential topology" {
...
@test "§19.4.1 documents coverage delta (411->417 / 56->57 / ~1082->~1094)" {
...
@test "zero new architecture WARNINGs beyond pre-existing 2/12/13/16/10" {
...
@test "zero NEW WARNINGs naming epics-and-stories.md" {
  local script="$PROJECT_ROOT/gaia-public/plugins/gaia/scripts/check-monolith-shard-sync.sh"
  [ -x "$script" ] || skip "check-monolith-shard-sync.sh not present"
  cd "$PROJECT_ROOT"
  run "$script"
...
@test "cascade narrative carries audit note flagging" {
...
@test "exactly one ADR row references" {
  local count
