...
@test "SKILL.md contains Relationship to Existing Testing Skills section" {
  grep -q "Relationship to Existing Testing Skills" "$SKILL_MD"
}

# ---------- AC4: All 6 sibling skills named ----------

@test "Relationship section names gaia-test-e2e" {
  grep -q "gaia-test-e2e" "$SKILL_MD"
}

@test "Relationship section names gaia-test-mobile-e2e" {
  grep -q "gaia-test-mobile-e2e" "$SKILL_MD"
}

@test "Relationship section names gaia-test-a11y" {
  grep -q "gaia-test-a11y" "$SKILL_MD"
}

@test "Relationship section names gaia-review-mobile" {
  grep -q "gaia-review-mobile" "$SKILL_MD"
}

@test "Relationship section names gaia-test-device-matrix" {
  grep -q "gaia-test-device-matrix" "$SKILL_MD"
}

@test "Relationship section names gaia-config-device-target" {
  grep -q "gaia-config-device-target" "$SKILL_MD"
}

# ---------- AC4: No PLUGIN_DIR reference ----------

@test "SKILL.md does not reference PLUGIN_DIR variable" {
  ! grep -q 'PLUGIN_DIR' "$SKILL_MD"
}

# ---------- AC4: Supported Surfaces section exists ----------

@test "SKILL.md contains Supported Surfaces section" {
  grep -q "Supported Surfaces" "$SKILL_MD"
}

# ---------- AC4: Step 1b surface resolution documented ----------

@test "SKILL.md documents surface profile resolution step" {
