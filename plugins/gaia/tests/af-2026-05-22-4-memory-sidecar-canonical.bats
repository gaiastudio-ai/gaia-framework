...
@test "gaia-create-arch SKILL.md architecture-decisions writes to canonical .gaia/memory/" {
...
@test "gaia-meeting SKILL.md sidecar paths canonical" {
  grep -qF '.gaia/memory/{agent}-sidecar' "$PLUGIN_ROOT/skills/gaia-meeting/SKILL.md"
}

@test "gaia-brownfield SKILL.md sidecar writes canonical" {
...
@test "gaia-memory-hygiene SKILL.md main-context read target canonical (with legacy fallback noted)" {
...
@test "dual-layout caveats preserved (lines mentioning both legacy + canonical)" {
...
@test "framework-wide — no bare _memory/<agent>-sidecar write targets in SKILL.md prose" {
