...
@test "Skill Classification section enumerates >=16 canonical skills" {
  [ -n "$ADR_FILE" ]
...
@test "Bypass Vocabulary names --bypass, --reason, per-sprint, append-only" {
  [ -n "$ADR_FILE" ]
...
@test "Recording Schema names all 5 YAML fields" {
  [ -n "$ADR_FILE" ]
