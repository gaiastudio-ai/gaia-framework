...
@test "clarify mode resolves from the registry" {
...
@test "aliases clarification and questions canonicalize to clarify" {
...
# mode name or alias.
@test "clarify name and aliases do not collide" {
...
@test "select-notes-template resolves clarification-notes bias" {
...
@test "clarification-notes template exists" {
  [ -f "$TEMPLATE" ]
  [ -s "$TEMPLATE" ]
}

# TC-MTG-CLARIFY-6 — template does NOT contain decision-record headings.
@test "template has no decision-record headings" {
...
@test "template has no brainstorming-document headings" {
  [ -f "$TEMPLATE" ]   # guard: a missing file would make the grep vacuously pass
