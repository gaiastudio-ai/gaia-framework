...
@test "gaia-create-ux SKILL.md Step 9 declares Import mode is read-only" {
  run grep -E "read-only|read only|zero write|expected_writes" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md Step 9 declares expected_writes: 0 for Import mode" {
  run grep -E "expected_writes:[[:space:]]*0|expected_writes = 0" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md Step 9 declares allowed_write_calls" {
  run grep -E "allowed_write_calls:[[:space:]]*\[\]" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "Import mode emits audit table with the canonical column set" {
...
@test "Import mode declares an Compliance Audit section under Step 9" {
...
@test "gaia-create-ux SKILL.md documents pre-dispatch write guard" {
  run grep -E "pre-dispatch|pre dispatch|short-circuit|short circuit" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents 'write / blocked' audit classification" {
  run grep -F "write / blocked" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md surfaces an violation message on attempted write" {
  run grep -E "Read-only violation|compliance violation" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md references at least one canonical write call name" {
...
@test "gaia-create-ux SKILL.md references at least one canonical read call name in Step 9" {
...
@test "gaia-create-ux SKILL.md documents file-key validation step" {
  run grep -E "file key validation|File key validation|validateFigmaFileKey" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents halt-before-API on invalid key" {
  run grep -E "halt before|before any|reject malformed" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "figma-integration SKILL.md exposes validateFigmaFileKey helper contract" {
  run grep -F "validateFigmaFileKey" "$FIGMA_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "figma-integration SKILL.md documents the canonical 22+ char key pattern" {
...
@test "gaia-create-ux SKILL.md documents depth-1 metadata check" {
  run grep -E "depth-1|depth=1|depth 1" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md records lastModified in metadata block" {
  run grep -F "lastModified" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md records version identifier in metadata block" {
...
@test "figma-integration SKILL.md exposes classifyViewport helper" {
  run grep -F "classifyViewport" "$FIGMA_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md Step 9 lists the canonical viewport set 280/375/600/768/1024/1280" {
...
@test "gaia-create-ux SKILL.md documents the 'custom' viewport bucket" {
  run grep -E "\bcustom\b" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents viewport distribution table" {
...
@test "gaia-create-ux SKILL.md documents W3C DTCG token format" {
  run grep -E "W3C DTCG|DTCG" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md references DTCG \$value key" {
  run grep -F '$value' "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md references DTCG \$type key" {
  run grep -F '$type' "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md references DTCG \$description key" {
  run grep -F '$description' "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md references the design-tokens.json output path" {
...
@test "gaia-create-ux SKILL.md references component-specs.yaml output path" {
  run grep -F "component-specs.yaml" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents schema_version field for component-specs.yaml" {
  run grep -F "schema_version" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents COMPONENT and COMPONENT_SET node filtering" {
  run grep -E "COMPONENT_SET|COMPONENT\b" "$UX_SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "gaia-create-ux SKILL.md documents platform_tokens placeholder field" {
...
@test "gaia-create-ux SKILL.md keeps exactly 12 top-level Step headings after Import restoration" {
...
@test "Subtask 8.2: gaia-create-ux SKILL.md cross-references architecture §10.17 for" {
