...
@test "F-H01: sarif-merge.sh no longer passes --force on the active sarif merge call lines" {
...
@test "F-H01: sarif-merge.sh uses --log ForceOverwrite (canonical Sarif.Multitool knob)" {
  run grep -F -- '--log ForceOverwrite' "$PLUGIN_ROOT/scripts/adapters/brownfield/sarif-merge.sh"
  [ "$status" -eq 0 ]
}

@test "F-M01: sarif-merge.sh passes --merge-empty-logs (per-tool run survives a clean scan)" {
  run grep -F -- '--merge-empty-logs' "$PLUGIN_ROOT/scripts/adapters/brownfield/sarif-merge.sh"
  [ "$status" -eq 0 ]
}

@test "F-H01: sarif-merge.sh passes inputs FIRST (container-inputs before --output-directory)" {
...
@test "F-M02: orchestrator matches_glob treats bare-dir / trailing-slash as <dir>/**" {
  run grep -F 'dir prefix' "$PLUGIN_ROOT/scripts/adapters/brownfield/orchestrator.sh"
  [ "$status" -eq 0 ]
}

@test "F-M02: orchestrator cites Test16 F-M02 + names the symptom (0 files vs 37)" {
...
@test "F-M02: gaia-init generate-config.sh normalizes paths to <dir>/** before persistence" {
...
@test "F-M03: readiness generator emits Completeness/Consistency/Contradictions/Cascades sections" {
...
@test "F-M03: readiness generator emits traceability_complete + test_implementation_rate in frontmatter" {
...
@test "F-M04: generate-frontmatter.sh awk pattern accepts colon AND em-dash heading forms" {
...
@test "F-M04: generate-frontmatter.sh derives epic from story-key prefix when Epic bullet is absent" {
...
@test "F-M05: sprint-state.sh dep resolver tier 2 reads the depended story file's frontmatter" {
  run grep -F 'Tier 2: the depended story' "$PLUGIN_ROOT/scripts/sprint-state.sh"
  [ "$status" -eq 0 ]
}

@test "F-M05: sprint-state.sh dep resolver tier 3 walks sprint-archive/" {
  run grep -F 'Tier 3: scan sprint-archive' "$PLUGIN_ROOT/scripts/sprint-state.sh"
  [ "$status" -eq 0 ]
}

@test "F-M05: sprint-state.sh wrapper is byte-identical with the canonical (per feedback memory)" {
...
@test "F-L01: Dockerfile bakes GAIA_SPOTBUGS_VERSION into runtime ENV" {
  run grep -F 'GAIA_SPOTBUGS_VERSION' "$PLUGIN_ROOT/tools/gaia-tools/Dockerfile"
  [ "$status" -eq 0 ]
}

@test "F-L01: entrypoint prefers GAIA_SPOTBUGS_VERSION env over the broken launcher probe" {
  run grep -F 'GAIA_SPOTBUGS_VERSION' "$PLUGIN_ROOT/tools/gaia-tools/gaia-tools-entrypoint.sh"
  [ "$status" -eq 0 ]
}

@test "F-L02: gaia-doctor/setup.sh has no UNCOMMENTED 5-levels-up walk" {
...
@test "F-L02: gaia-doctor/setup.sh asks resolve-config.sh project_root" {
  run grep -F 'resolve-config.sh' "$PLUGIN_ROOT/skills/gaia-doctor/scripts/setup.sh"
  [ "$status" -eq 0 ]
}

@test "F-L03: tool-readiness.json grype version_cmd has a multi-tier fallback" {
  run grep -F 'awk -F' "$PLUGIN_ROOT/skills/gaia-doctor/knowledge/tool-readiness.json"
  [ "$status" -eq 0 ]
}

@test "F-L04: gaia-init .gitignore seed names pytest artifacts" {
...
@test "F-L05: sprint dashboard reads canonical start_date and no longer reads the retired capacity proxy" {
...
@test "F-L06: backfill-story-index.sh exists and is executable" {
  [ -x "$PLUGIN_ROOT/scripts/backfill-story-index.sh" ]
}

@test "F-L06: /gaia-sprint-plan Step 1 invokes backfill-story-index.sh" {
  run grep -F 'backfill-story-index.sh' "$PLUGIN_ROOT/skills/gaia-sprint-plan/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "F-L07: sprint-state.sh init emits a sprint-plan/{id}-plan.md stub" {
  run grep -F 'sprint-plan stub' "$PLUGIN_ROOT/scripts/sprint-state.sh"
  [ "$status" -eq 0 ]
}

@test "F-L08: install-test-environment-example.sh mkdir-p the test-artifacts mirror dir" {
  run grep -F 'mkdir -p "$_mirror_test_artifacts"' "$PLUGIN_ROOT/scripts/install-test-environment-example.sh"
  [ "$status" -eq 0 ]
}

@test "F-L09 + F-L10: brownfield SKILL documents the test-lens mirror as design intent" {
...
@test "F-L11: gaia-test-strategy finalize emits a test-strategy.md stub when only test-plan.md exists" {
...
@test "D-1: gaia-config-brownfield troubleshooting now names tools.image" {
  run grep -F 'tools.image' "$REPO_ROOT/documentation/commands/gaia-config-brownfield.html"
  [ "$status" -eq 0 ]
}

@test "D-2: brownfield tutorial Caveats warns about the /gaia-init exit-1 stub" {
...
@test "D-3: docker-workflow doc lists the current image version (no longer 0.1.1 / 2026-05-31)" {
