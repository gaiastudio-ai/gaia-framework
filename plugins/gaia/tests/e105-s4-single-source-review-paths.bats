...
@test "AC3/TS3: **Verdict:** V (colon-outside-bold) parses to the value" {
  [ "$(verdict_of "$FX/shape-bold-colon-outside.md")" = "PASSED" ]
}

@test "AC3/TS3: **Verdict: V** (colon-inside-bold, the dominant form) parses to the value" {
  [ "$(verdict_of "$FX/shape-bold-colon-inside.md")" = "PASSED" ] \
    || { echo "got: $(verdict_of "$FX/shape-bold-colon-inside.md")" >&2; false; }
}

@test "AC3/TS3: ## Verdict: V (H2 heading form, e.g. E106-S1) parses to the value" {
  [ "$(verdict_of "$FX/shape-heading.md")" = "PASSED" ] \
    || { echo "got: $(verdict_of "$FX/shape-heading.md")" >&2; false; }
}

@test "AC3/TS3: plain Verdict: V parses to the value" {
  [ "$(verdict_of "$FX/shape-plain.md")" = "PASSED" ] \
    || { echo "got: $(verdict_of "$FX/shape-plain.md")" >&2; false; }
}

@test "AC3/TS3: arrow-override **Verdict: X -> V** takes the POST-arrow value" {
  v="$(verdict_of "$FX/shape-arrow-override.md")"
  [ "$v" = "PASSED" ] \
    || { echo "arrow-override should yield the post-arrow PASSED, got: $v" >&2; false; }
}

@test "AC3/TS3: a value-vocabulary variant (REQUEST_CHANGES) parses (not UNKNOWN)" {
  v="$(verdict_of "$FX/shape-vocab-request-changes.md")"
  [ "$v" = "REQUEST_CHANGES" ] \
    || { echo "expected REQUEST_CHANGES, got: $v" >&2; false; }
}

@test "AC3/TS3: a genuinely missing verdict still yields UNKNOWN" {
  [ "$(verdict_of "$FX/shape-missing.md")" = "UNKNOWN" ]
}

@test "AC3 F1: gate-row annotation (APPROVE -> Review Gate row = PASSED) keeps the BASE verdict, not the post-arrow word" {
...
@test "AC3 regression: pre-existing **Verdict:** V branch is preserved (no regression)" {
...
@test "AC3/TS5: review-extract over a dir of mixed-shape reports yields real verdicts, no UNKNOWN" {
...
@test "AC1: gaia-run-all-reviews SKILL.md is the declared single-source path table" {
  [ -f "$TABLE" ]
...
@test "AC2: the canonical table references the per-story reviews/ subdir (E105-S1 layout)" {
  [ -f "$TABLE" ]
...
@test "AC1/AC4: review-summary-gen.sh resolves report paths reflecting reviews/ (no flat-only divergence)" {
  [ -f "$SUMMARY" ]
...
@test "AC2: review reports keep FR-402 type-FIRST names (no {key}-<type> reversal)" {
