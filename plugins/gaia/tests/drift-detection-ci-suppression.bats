...
@test "Security: GAIA_SKIP_VERSION_CHECK truthy-aliases ('true', 'yes') do NOT trigger skip" {
  # Per AC5, only the literal string "1" triggers the skip. Common truthy
  # aliases ("true", "yes", "TRUE") must NOT match — this prevents cargo-cult
  # bypass values from accidentally suppressing the check. Asserted via a
  # single test against both aliases (CI-budget conscious).
  write_drifted_config
  run_resolver_env "GAIA_SKIP_VERSION_CHECK=true"
  [ "$status" -eq 0 ]
  [ -f "$FIXTURE_DIR/_memory/.framework-version-stale" ]
  rm -f "$FIXTURE_DIR/_memory/.framework-version-stale" \
        "$FIXTURE_DIR/_memory/.framework-version-checked-"*
