...
@test "retro-a: SKILL.md references nested retrospective path" {
  [ -f "$SKILL" ]
  grep -qF "implementation-artifacts/retrospective/retrospective-" "$SKILL"
}

@test "retro-b: no script writes to legacy flat retrospective path" {
  [ -d "$SCRIPTS_DIR" ]
...
@test "retro-c: SKILL.md includes mkdir -p guidance for nested directory" {
  [ -f "$SKILL" ]
  grep -qF "mkdir -p" "$SKILL"
}

@test "retro-d: documented write target ends with retrospective-{sprint_id}-{date}.md under retrospective/" {
  [ -f "$SKILL" ]
