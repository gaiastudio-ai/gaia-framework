...
setup() {
  ...
  LIB="$PLUGIN/scripts/lib/lifecycle-overrides.sh"
  SCHEMA="$PLUGIN/schemas/lifecycle-overrides.schema.json"
  PARSER="$PLUGIN/scripts/lib/parse-bypass-flag.sh"
  TMP_ROOT="$(mktemp -d)"
...
@test "schema file exists and declares required fields" {
  [ -f "$SCHEMA" ]
...
@test "lifecycle_read_bypasses on absent file returns empty bypasses" {
...
@test "append rejects missing --reason" {
...
@test "append rejects short --reason" {
...
@test "append bootstraps absent file then appends second record" {
...
@test "10 parallel appends produce 10 unique entries" {
  pids=()
...
@test "list filters by sprint and emits table format" {
...
@test "list --format json emits valid JSON parseable by jq" {
...
@test "parse-bypass-flag rejects --bypass without --reason" {
  run bash "$PARSER" --bypass /gaia-trace
  [ "$status" -ne 0 ]
  [[ "$output" == *"--reason"* ]]
}

@test "parse-bypass-flag exports BYPASS_SKILL and BYPASS_REASON" {
