...
@test "#1047: schema declares sprint_review.frontend_commands (map)" {
  run jq -r '.properties.sprint_review.properties.frontend_commands.type' "$SCHEMA"
  [ "$status" -eq 0 ]
  [ "$output" = "object" ]
}

@test "#1047: frontend_commands additionalProperties enforces string command-values" {
  run jq -r '.properties.sprint_review.properties.frontend_commands.additionalProperties.type' "$SCHEMA"
  [ "$status" -eq 0 ]
  [ "$output" = "string" ]
}

@test "#1047: frontend_commands description names the four sibling maps" {
...
@test "#1047: legacy frontend_command (scalar) stays in the schema as deprecated alias" {
  run jq -r '.properties.sprint_review.properties.frontend_command.type' "$SCHEMA"
  [ "$status" -eq 0 ]
  [ "$output" = "string" ]
}

@test "#1047: deprecated frontend_command description names the canonical map alternative" {
...
@test "#1047: track-b-dispatch.sh collects stacks_frontend from frontend_commands" {
  run grep -F "stacks_frontend=" "$DISPATCH"
  [ "$status" -eq 0 ]
}

@test "#1047: track-b-dispatch.sh stack_command_for resolves via frontend_commands first" {
...
@test "#1047: track-b-dispatch.sh emits a DEPRECATED advisory when the scalar is used and the map lacks 'frontend'" {
  run grep -F "DEPRECATED: sprint_review.frontend_command" "$DISPATCH"
  [ "$status" -eq 0 ]
}

@test "#1047: track-b-dispatch.sh merge_legacy_frontend_scalar gate blocks double-add on key collision" {
...
@test "#1047: gaia-config-sprint-review SKILL.md names frontend_commands.<stack-id>" {
  run grep -F 'frontend_commands.<stack-id>' "$CONFIG_SKILL"
  [ "$status" -eq 0 ]
}

@test "#1047: gaia-config-sprint-review SKILL.md flags frontend_command as deprecated" {
...
@test "#1047: a project with two web stacks (frontend + website) validates against the schema" {
