...
#   2. version-bump.js minor dry-run produces v1.136.0 (current is 1.135.0).
...
@test "version-bump.js minor dry-run reports 1.136.0" {
  cd "$REPO_ROOT"
  run node scripts/version-bump.js minor --dry-run
  [ "$status" -eq 0 ]
  echo "$output" | grep -q '1.135.0 -> 1.136.0'
}
...
@test "plugin.json current version is 1.135.0" {
  PLUGIN="${REPO_ROOT}/plugins/gaia/.claude-plugin/plugin.json"
  [ -f "$PLUGIN" ]
  grep -q '"version": "1.135.0"' "$PLUGIN"
}
