...
@test "Dockerfile runtime stage apt-installs libicu72" {
...
@test "Dockerfile installs Sarif.Multitool (not Microsoft.Sarif.Multitool)" {
  run grep -F 'dotnet tool install --tool-path /usr/local/bin Sarif.Multitool' \
    "$PLUGIN_ROOT/tools/gaia-tools/Dockerfile"
  [ "$status" -eq 0 ]
}

@test "Dockerfile dotnet tool install line does not name Microsoft.Sarif.Multitool" {
...
@test "Dockerfile sets ENV DOTNET_ROOT=/opt/dotnet" {
...
@test "grype adapter docker dispatch does not use --output json on db status" {
...
@test "grype adapter parses Checksum: from plain-text db status" {
...
@test "sarif-merge.sh stages inputs into /out/.merge-in-\$\$/" {
  run grep -F '.merge-in-$' "$PLUGIN_ROOT/scripts/adapters/brownfield/sarif-merge.sh"
  [ "$status" -eq 0 ]
}

@test "sarif-merge.sh passes container-relative /out paths to sarif merge" {
  run grep -F '/out/.merge-in-' "$PLUGIN_ROOT/scripts/adapters/brownfield/sarif-merge.sh"
  [ "$status" -eq 0 ]
}

@test "brownfield SKILL Phase-3 grype call sets ADAPTER_OUT_DIR=\$AUDIT/sarif" {
...
@test "sarif merge invoked with --force (idempotent re-runs)" {
...
@test "brownfield-assessment template frontmatter has project_type" {
...
@test "readiness-report generator emits contradictions_found in frontmatter" {
  run grep -F 'contradictions_found' \
    "$PLUGIN_ROOT/skills/gaia-readiness-check/scripts/generate-readiness-report.sh"
  [ "$status" -eq 0 ]
}

@test "readiness-report generator emits ## Output Verification body section" {
...
@test "brownfield SKILL Known constraint cites planning-artifacts home for test-plan" {
...
@test "review-gate.sh inspects exit_code on execution-evidence" {
  run grep -F 'exit_code' "$PLUGIN_ROOT/scripts/review-gate.sh"
  [ "$status" -eq 0 ]
}

@test "review-gate.sh inspects fail_count on execution-evidence" {
  run grep -F 'fail_count' "$PLUGIN_ROOT/scripts/review-gate.sh"
  [ "$status" -eq 0 ]
}

@test "review-gate.sh refuses PASSED verdict when execution-evidence is RED" {
...
@test "F-: prd-template carries Scan-prefix → heading legend" {
  run grep -F 'Scan-prefix' "$PLUGIN_ROOT/skills/gaia-create-prd/prd-template.md"
  [ "$status" -eq 0 ]
}

@test "F-: prd-template anchors a severity-vocabulary table" {
  run grep -iF 'severity vocabulary' "$PLUGIN_ROOT/skills/gaia-create-prd/prd-template.md"
  [ "$status" -eq 0 ]
}

@test "F-: prd-template carries an output_path frontmatter line" {
...
@test "L-01: docker-runner.sh BASH_SOURCE has :- default guard" {
...
@test "L-02: entrypoint spotbugs --version awk parses last field" {
...
@test "L-03: check-tools.sh --json emits a runner field" {
...
@test "L: review-gate.sh references review-summary.md emission" {
...
@test "L: brownfield SKILL documents .gaia/state/ as sprint-status canonical home" {
...
@test "L: review-perf SKILL documents performance-review-{key}.md file naming" {
  run grep -F 'performance-review-{story_key}.md' "$PLUGIN_ROOT/skills/gaia-review-perf/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "L: review-perf SKILL clarifies SKILL slug vs FILE name distinction" {
...
@test "L: install-test-environment-example.sh mirrors into test-artifacts/" {
...
@test "L: resolver new path is epic-{slug}/{key}-{slug}/{type}.md (no stories/)" {
  run grep -F 'epic-{slug}/{key}-{slug}/{type}.md' \
    "$PLUGIN_ROOT/scripts/lib/resolve-test-artifact-per-story.sh"
  [ "$status" -eq 0 ]
}

@test "L: resolver retains legacy stories/ path as read-compat fallback" {
...
@test "D-1: docker-workflow doc names libicu72 in build prerequisites" {
  run grep -F 'libicu72' \
    "$REPO_ROOT/documentation/commands/gaia-docker-workflow.html"
  [ "$status" -eq 0 ]
}

@test "D-1: docker-workflow doc names DOTNET_ROOT in build prerequisites" {
...
@test "D-2: docker-workflow doc documents the Sarif.Multitool row + merge role" {
...
@test "D-3: init CI scaffold stub onboarding doc exists" {
  [ -f "$PLUGIN_ROOT/skills/gaia-init/docs/ci-scaffold-stub.html" ]
}

@test "D-3: init CI scaffold stub onboarding doc names /gaia-ci-setup" {
