...
@test "AF-21-28: project-config.yaml.example uses .gaia/artifacts/ paths" {
...
@test "AF-21-28: project-config.yaml.example contains NO bare-legacy docs/<type>-artifacts paths" {
  ! grep -qE '^(planning|implementation|test|creative)_artifacts:\s+docs/' "$PLUGIN_ROOT/config/project-config.yaml.example"
}

@test "AF-21-28: gaia-help.csv contains zero docs/<type>-artifacts/ refs (all canonicalized)" {
  ! grep -qE 'docs/(planning|test|creative|implementation|research)-artifacts' "$PLUGIN_ROOT/knowledge/gaia-help.csv"
}

@test "AF-21-28: gaia-help.csv canonical .gaia/artifacts refs present" {
