...
@test "Dockerfile installs .NET SDK channel 8.0 (not runtime-only)" {
  run grep -F -e '--channel 8.0' "$PLUGIN_ROOT/tools/gaia-tools/Dockerfile"
  [ "$status" -eq 0 ]
}

@test "Dockerfile drops the silent || echo WARNING swallow on sarif install" {
...
@test "check-tools.sh fallback projects .language//.name//.id" {
...
@test "brownfield-assessment-template uses schema_version 2.0.0" {
...
@test "gaia-ci-setup generate-pipeline.sh exists + is executable" {
  [ -x "$PLUGIN_ROOT/skills/gaia-ci-setup/scripts/generate-pipeline.sh" ]
}

@test "ci-setup SKILL.md wires the generator before LLM authoring" {
  run grep -F 'generate-pipeline.sh' "$PLUGIN_ROOT/skills/gaia-ci-setup/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "generate-pipeline.sh supports python stack" {
  cd "$TEST_TMP"
...
@test "sprint-state.sh has review→closed Val-sentinel guard" {
  run grep -F 'refuse review→closed' "$PLUGIN_ROOT/scripts/sprint-state.sh"
  [ "$status" -eq 0 ]
}

@test "GAIA_ALLOW_SPRINT_REVIEW_TO_CLOSED_WITHOUT_SENTINEL escape hatch documented" {
...
@test "gaia-init SKILL.md materialises the bundle to a tempfile first" {
...
@test "entrypoint spotbugs row uses SpotBugs awk pattern" {
...
@test "insert refuses wrapper/section mismatch" {
  run grep -F 'wrapper mismatch' "$PLUGIN_ROOT/scripts/config-yaml-editor.sh"
  [ "$status" -eq 0 ]
}

@test "insert end-to-end refuses unwrapped content" {
  cd "$TEST_TMP"
  cat > base.yaml <<'EOF'
project_root: /tmp/x
EOF
...
@test "grype adapter captures DB checksum from inside the container" {
...
@test "resolve-config.sh has parse_yaml_3deep" {
  run grep -F 'parse_yaml_3deep' "$PLUGIN_ROOT/scripts/resolve-config.sh"
  [ "$status" -eq 0 ]
}

@test "resolve-config.sh --field brownfield.tools.runner case" {
  run grep -F 'brownfield.tools.runner)' "$PLUGIN_ROOT/scripts/resolve-config.sh"
  [ "$status" -eq 0 ]
}

@test "resolve-config.sh --field brownfield.tools.image case" {
...
@test "render-test-quality.sh INFO-skips on missing report" {
...
@test "validate-canonical-filename.sh computes slugify(title)" {
  run grep -F '_expected_slug' "$PLUGIN_ROOT/skills/gaia-create-story/scripts/validate-canonical-filename.sh"
  [ "$status" -eq 0 ]
}

@test "validate-canonical-filename rejects slug drift" {
...
@test "gaia-sprint-review SKILL.md uses fully-qualified write-val-sentinel path" {
  run grep -F 'skills/gaia-sprint-review/scripts/write-val-sentinel.sh' "$PLUGIN_ROOT/skills/gaia-sprint-review/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "path-disambiguation note added at top of SKILL.md" {
...
@test "test-artifacts-mirror.sh helper exists + is executable" {
  [ -x "$PLUGIN_ROOT/scripts/lib/test-artifacts-mirror.sh" ]
}

@test "review-gate.sh hooks the mirror for test-lens gates" {
...
@test "test-environment-manifest.sh mirrors to test-artifacts/" {
...
@test "review-summary-gen.sh writes per-story reviews/ aggregator" {
...
@test "gaia-trace SKILL.md routes test-plan reads through planning-artifacts first" {
...
@test "brownfield Phase 9b documents standalone architecture adversarial" {
...
@test "D-01: brownfield SKILL.md spells out per-artifact destination split" {
  run grep -F 'Per-artifact destination split' "$PLUGIN_ROOT/skills/gaia-brownfield/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "D-02: brownfield Phase 6 routes headless projects at phase-5-for-non-deployable-projects" {
  run grep -F 'phase-5-for-non-deployable-projects.md' "$PLUGIN_ROOT/skills/gaia-brownfield/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "D-03: brownfield SKILL.md documents Phase 7 empty-scan grading + id-prefix convention" {
  run grep -F 'Empty-but-UNVERIFIED scan' "$PLUGIN_ROOT/skills/gaia-brownfield/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "D-04: brownfield SKILL.md names the PRD template plugin-relative location" {
  run grep -F 'skills/gaia-create-prd/prd-template.md' "$PLUGIN_ROOT/skills/gaia-brownfield/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "D-04: brownfield SKILL.md documents the 5-tier / 3-tier severity mapping" {
  run grep -F 'config-severity bucket' "$PLUGIN_ROOT/skills/gaia-brownfield/SKILL.md"
  [ "$status" -eq 0 ]
}

@test "D-05: test-strategy SKILL.md documents the two-artifact expectation + frontmatter" {
