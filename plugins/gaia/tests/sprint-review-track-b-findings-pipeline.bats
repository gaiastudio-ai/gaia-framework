...
@test "type-target-resolver rejects unknown types" {
...
@test "type-target-resolver resolves all 11 canonical types without error" {
...
@test "write-evidence creates run-record.md with readable content" {
...
@test "write-evidence creates exit-code.log with VERDICT line" {
...
@test "write-evidence exit-code.log contains the verdict value" {
...
@test "SKILL.md Step 7 references envelope → review-gate → action-items pipeline for manual-test findings" {
...
@test "SKILL.md documents action-items-writer.sh for sprint-correction type" {
...
@test "write-evidence --verify passes when both evidence files are present and non-empty" {
...
@test "write-evidence --verify fails when evidence is missing" {
