...
@test "SKILL.md lists /gaia-create-arch as an auto-incorporation caller" {
  run grep -F "/gaia-create-arch" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "SKILL.md lists /gaia-create-prd as an auto-incorporation caller" {
  run grep -F "/gaia-create-prd" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "SKILL.md lists /gaia-create-epics as an auto-incorporation caller" {
  run grep -F "/gaia-create-epics" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "SKILL.md lists /gaia-readiness-check as an auto-incorporation caller" {
  run grep -F "/gaia-readiness-check" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "combined grep -E matches all four caller tokens" {
...
@test "four caller bullets are contiguous in a single list" {
...
@test "SKILL.md declares the Step 4 Invocation Contract subsection" {
...
@test "SKILL.md states standalone-default rule for non-listed callers" {
...
@test "SKILL.md documents the incorporate: true opt-in signal" {
  run grep -F "incorporate: true" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "SKILL.md states both conditions are required (allowlist AND signal)" {
...
@test "Critical Rules contains a pointer bullet to the contract" {
...
@test "Critical Rules preserves the Do NOT suggest fixes rule verbatim" {
  run grep -F "Do NOT suggest fixes — only identify problems" "$SKILL_MD"
  [ "$status" -eq 0 ]
}

@test "Step 4 preserves the only-when-caller-requested line" {
