...
#       name.
# ---------------------------------------------------------------------------
@test "apple-app-store.json exists at canonical regimes path" {
  [ -f "$RUBRIC" ]
}

@test "apple-app-store.json declares schema_version 1.0 and name=apple-app-store" {
...
# ---------------------------------------------------------------------------
@test "at least one AAS-HIG- prefixed rule exists" {
  n=$(jq '[.severity_rules[] | select(.id | startswith("AAS-HIG-"))] | length' "$RUBRIC")
  [ "$n" -ge 6 ] || { echo "AAS-HIG- rule count=$n (expected >=6)" >&2; return 1; }
}

@test "AAS-HIG covers touch-target / nav / tab / safe-area / dynamic-type / sf-symbols" {
...
@test "at least one AAS-IAP- prefixed rule exists" {
  n=$(jq '[.severity_rules[] | select(.id | startswith("AAS-IAP-"))] | length' "$RUBRIC")
  [ "$n" -ge 5 ] || { echo "AAS-IAP- rule count=$n (expected >=5)" >&2; return 1; }
}

@test "AAS-IAP covers receipt-validation / storekit / subscription / restore / external-payment" {
...
@test "at least one AAS-PRIV- prefixed rule exists" {
  n=$(jq '[.severity_rules[] | select(.id | startswith("AAS-PRIV-"))] | length' "$RUBRIC")
  [ "$n" -ge 4 ] || { echo "AAS-PRIV- rule count=$n (expected >=4)" >&2; return 1; }
}

@test "AAS-PRIV covers manifest / required-reason / label-consistency / third-party-sdk" {
...
@test "at least one AAS-ATT- prefixed rule exists" {
  n=$(jq '[.severity_rules[] | select(.id | startswith("AAS-ATT-"))] | length' "$RUBRIC")
  [ "$n" -ge 4 ] || { echo "AAS-ATT- rule count=$n (expected >=4)" >&2; return 1; }
}

@test "AAS-ATT covers tracking-authorization / usage-description / denial-handling / fingerprinting" {
...
@test "3.1.1 IAP and 5.1 privacy guideline violations are Critical" {
...
@test "AAS-* rule ids do not collide with the nine regime rubrics" {
...
# ---------------------------------------------------------------------------
@test "rubric-merger.sh merges with apple-app-store as later layer" {
  if [ ! -x "$MERGER" ]; then
