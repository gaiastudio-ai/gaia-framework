#!/usr/bin/env bats
# evidence-judgment-parity.bats — per-skill parity bats for gaia-review-a11y
# (E75-S2, FR-RSV2-46, ADR-075).
#
# Tier 1: evidence — axe-core + pa11y WCAG check output
# Tier 2: judgment — accessibility findings with severities + WCAG criterion ids
...
@test "${SKILL_NAME}: tier 1 — evidence collection produces structured analysis-results JSON" {
  ...
  [ "$status" -eq 0 ]
  printf '%s\n' "$output" | grep -qE 'axe-core|pa11y'
}

@test "${SKILL_NAME}: tier 2 — judgment maps evidence to findings with severities" {
  assert_judgment_findings "$FINDINGS"
  # Skill-specific: at least one finding references a WCAG criterion.
  run jq -e '.findings | any(.wcag != null)' "$FINDINGS"
  [ "$status" -eq 0 ]
}
