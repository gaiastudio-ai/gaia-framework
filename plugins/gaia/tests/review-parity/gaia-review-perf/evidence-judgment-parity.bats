#!/usr/bin/env bats
# evidence-judgment-parity.bats — per-skill parity bats for gaia-review-perf
# (E75-S2, FR-RSV2-46, ADR-075).
#
# Tier 1: evidence — profiling + benchmark metrics
# Tier 2: judgment — performance findings with severities
...
@test "${SKILL_NAME}: tier 1 — evidence collection produces structured analysis-results JSON" {
  ...
  [ "$status" -eq 0 ]
  printf '%s\n' "$output" | grep -qE 'lighthouse|clinic'
}
