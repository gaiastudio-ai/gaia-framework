...
@test "_schema/adapter.schema.json exists and is valid JSON" {
  [ -f "$ADAPTER_SCHEMA" ]
  jq -e . "$ADAPTER_SCHEMA" >/dev/null
}

@test "schema declares draft-07 (or later)" {
...
@test "'runtime-profile' enum is exactly subprocess|container|network" {
...
@test "schema validates semgrep/adapter.json with zero errors" {
  run _validate_adapter "$ADAPTERS_DIR/semgrep/adapter.json"
  if [ "$status" -eq 99 ]; then skip "python3 jsonschema not available"; fi
  [ "$status" -eq 0 ]
}

@test "schema validates gitleaks/adapter.json with zero errors" {
  run _validate_adapter "$ADAPTERS_DIR/gitleaks/adapter.json"
  if [ "$status" -eq 99 ]; then skip "python3 jsonschema not available"; fi
  [ "$status" -eq 0 ]
}

@test "schema validates radon/adapter.json with zero errors" {
  run _validate_adapter "$ADAPTERS_DIR/radon/adapter.json"
  if [ "$status" -eq 99 ]; then skip "python3 jsonschema not available"; fi
  [ "$status" -eq 0 ]
}

@test "schema validates gocyclo/adapter.json with zero errors" {
  run _validate_adapter "$ADAPTERS_DIR/gocyclo/adapter.json"
  if [ "$status" -eq 99 ]; then skip "python3 jsonschema not available"; fi
  [ "$status" -eq 0 ]
}

@test "schema validates eslint-plugin-sonarjs/adapter.json with zero errors" {
...
@test "schema rejects adapter.json missing 'provider'" {
...
@test "run-contract.md references analysis-results.schema.json" {
  [ -f "$RUN_CONTRACT" ]
  grep -q 'analysis-results.schema.json' "$RUN_CONTRACT"
}

@test "adapter schema references analysis-results.schema.json via \$ref" {
...
@test "run-contract.md documents required flags" {
  [ -f "$RUN_CONTRACT" ]
...
@test "run-contract.md documents stdout/stderr contract" {
  grep -qi 'stdout' "$RUN_CONTRACT"
  grep -qi 'stderr' "$RUN_CONTRACT"
}

@test "run-contract.md documents exit code semantics" {
  grep -q '0.*success' "$RUN_CONTRACT" || grep -q 'exit.*0' "$RUN_CONTRACT"
  grep -q 'ran_and_errored' "$RUN_CONTRACT"
}

@test "run-contract.md documents timeout enforcement (SIGTERM/SIGKILL)" {
...
@test "run-contract.md documents all four probe states" {
...
@test "run-contract.md references probe-output.schema.json" {
  grep -q 'probe-output.schema.json' "$RUN_CONTRACT"
}

@test "run-contract.md documents probe JSON output shape (4 keys after )" {
...
@test "BOUNDARIES.md still contains existing content" {
  grep -q 'Tool Adapter' "$BOUNDARIES" || grep -q 'tool integrations' "$BOUNDARIES"
}

@test "BOUNDARIES.md references _schema/adapter.schema.json" {
  grep -q '_schema/adapter.schema.json' "$BOUNDARIES"
}

@test "BOUNDARIES.md references _schema/test/contract.bats template" {
  grep -q '_schema/test/contract.bats' "$BOUNDARIES"
}

@test "BOUNDARIES.md documents four-state probe vocabulary" {
  for state in available expected_and_missing ran_and_errored not_applicable; do
