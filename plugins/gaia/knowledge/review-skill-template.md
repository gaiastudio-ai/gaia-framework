...
Two runs against unchanged `analysis-results.json` MUST produce findings that match by category and severity.
...
- Invoke `plugins/gaia/scripts/load-stack-persona.sh` in the parent context — lazy-loads the matching reviewer persona + memory sidecar BEFORE fork dispatch (fork no-write isolation preserved).
...
- Surface divergence Warning to the user; do NOT block. Story Gate semantics are advisory.
...
Cache: cache key = `hash(File List contents + tool config + tool versions + resolved-config hash)`. Cache lives at `.gaia/state/review/{family}/{story_key}/.cache/` and plugs into `checkpoint.sh`.
...
Invoke `plugins/gaia/scripts/verdict-resolver.sh --analysis-results <path> --llm-findings <path>`. The resolver applies strict first-match-wins precedence:
...
- Render the report at `.gaia/artifacts/implementation-artifacts/{story_key}-review.md` (or per the naming convention) with two top-level sections:
...
- Surface the verdict back to the orchestrator (mandatory verdict surfacing).
...
## `.gaia/state/review/` Concurrency Requirement

The schema, resolver, and divergence/persona scripts ship first. The `.gaia/state/review/{family}/{story_key}/` directory itself is consumed when the cache plumbing lands. When the cache is wired:
...
This requirement is documented up front and implemented when the cache lands. Later consumers inherit the documented contract.

## Parity Suite

`evidence-judgment-parity.bats` enforces no-drift across the six review skills. The empty-loop SKIP-with-message protocol is mandatory for the zero-consumer state (template ships, no consumer merged yet) — the suite MUST NOT silently pass with zero assertions. As each consumer migrates, append its SKILL.md path to the `REVIEW_SKILLS` array.
...
# Consumers: populated as each review skill migrates
