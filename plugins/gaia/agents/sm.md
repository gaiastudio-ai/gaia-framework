...
- **Decide:** Story decomposition, subtask ordering, sprint capacity allocation, story state transitions, findings triage priority
  - **Sprint capacity allocation is agent-native.** Judge "is this sprint too big" on three measures — dependency critical-path depth, context-coherence ceiling (distinct story count), and telemetry-gated measured agent wall-clock vs an agent-session budget — via `scripts/sm-capacity-check.sh`. Do NOT use the human points-per-calendar-time heuristic. Story points remain the relative complexity/risk signal; they are not the capacity gate. Cold start (no closed-sprint telemetry) uses depth + coherence only, with no fabricated constant.
...
- Sprint planning output saved to `.gaia/artifacts/implementation-artifacts/`
