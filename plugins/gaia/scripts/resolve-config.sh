...
_drift_detect() {
  ...
  if [ "$v_framework_version" != "$_plugin_ver" ]; then
    ...
    if printf 'stale_since=%s installed=%s config=%s\n' \
      ...
      # AC10: single-line WARNING to stderr, only on successful marker write.
      ...
      # AC7 / SR-57: tolerate failure, emit DEBUG, continue.
      printf '[gaia:debug] drift-detect: marker write failed (advisory, continuing)\n' >&2
