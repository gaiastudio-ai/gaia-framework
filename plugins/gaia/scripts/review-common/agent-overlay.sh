...
#                           angular-dev, bash-dev, embedded-dev. Ignored for non-stack skills.
...
Options:
  ...
  --stack <stack>         Required for gaia-review-code only. One of:
                          ...
                          mobile-dev, angular-dev, bash-dev, embedded-dev.
...
is_canonical_stack() {
  case "$1" in
    ts-dev|java-dev|python-dev|go-dev|flutter-dev|mobile-dev|angular-dev|bash-dev|embedded-dev) return 0 ;;
    *) return 1 ;;
  esac
}
...
case "$SKILL" in
  gaia-review-code)
    ...
    [ -n "$STACK" ] || die 1 "--stack required for skill 'gaia-review-code' (one of: ts-dev, java-dev, python-dev, go-dev, flutter-dev, mobile-dev, angular-dev, bash-dev, embedded-dev)"
    is_canonical_stack "$STACK" || die 1 "invalid stack '$STACK' for gaia-review-code (expected one of: ts-dev, java-dev, python-dev, go-dev, flutter-dev, mobile-dev, angular-dev, bash-dev, embedded-dev)"
