#!/usr/bin/env bash
# adapters/plugin-frontmatter-validator/probe.sh — tri-state probe.
#
...
# (frontmatter parsing) and `jq` (JSON emission). Both are part of the GAIA
# baseline runtime — `awk` is universally available on POSIX systems and `jq`
# is required by every adapter. This probe checks `awk` (the
...
#
# This probe is invoked by the generalized tool-availability-probe.sh
