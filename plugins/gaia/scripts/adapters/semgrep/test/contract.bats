#!/usr/bin/env bats
# adapters/semgrep/test/contract.bats — ADR-078 adapter parity (E66-S2, NFR-RSV2-11).
...
@test "semgrep contract: adapter.json + run.sh present and well-formed" {
  assert_files_exist
}

@test "semgrep contract: state=available when tool on PATH and matching files" {
  local ext; ext="$(_contract_first_ext)"
  assert_state "$(_contract_provider)" available "$ext" 0 "" 0
  assert_fragment_shape
}

@test "semgrep contract: state=expected_and_missing when tool absent from PATH" {
  local ext; ext="$(_contract_first_ext)"
  assert_state "$(_contract_provider)" expected_and_missing "$ext" 0 "" 0
  assert_fragment_shape
}

@test "semgrep contract: state=ran_and_errored when run.sh exits non-zero" {
  local ext; ext="$(_contract_first_ext)"
  assert_state "$(_contract_provider)" ran_and_errored "$ext" 1 "scanner crash" 0
  assert_fragment_shape
}

@test "semgrep contract: state=not_applicable when file-list has no matching extensions" {
