#!/usr/bin/env bash
# adapters/lighthouse/run.sh — adapter contract for Google Lighthouse.
#
...
#
# Two additive flags extend the canonical contract:
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
adapters/lighthouse/run.sh — contract entry for Lighthouse.
Usage:
  run.sh --input <file-list> [--config <path>] [--output <path>]
         [--runtime-profile subprocess|container|network] [--timeout <seconds>]
         [--target-url <url>] [--categories <csv>]
