#!/usr/bin/env bash
# adapters/dead-code/render-test-quality.sh — unified Test Quality renderer.
#
...
# the contract this renderer exists to preserve.
#
...
# Degrade gracefully when the consolidated-gaps.md report doesn't exist yet.
# The prior `die "report not found"` made this script ordering-sensitive
# against the LLM consolidation step that creates the report — orderings that
# ran render-test-quality before consolidation died with a confusing error
# instead of just skipping the render. INFO-skip preserves the graceful-degrade
# contract every other brownfield adapter follows ("never aborts").
if [ ! -f "$REPORT" ]; then
...
# an explicit "no findings" line so the sub-section always renders.
render_subsection() {
...
mv "$body_tmp" "$REPORT"
# mktemp defaults to 0600 perms and `mv` preserves the source file's perms
# when crossing into the target. Force 0644 so consolidated-gaps.md matches
# the LLM-scan artifact convention (uniform-read across the brownfield-audit tree).
