...
This skill is Mode B-ready. Under the team-orchestration mode, any analysis subagent work that the prose above describes as inline subagent dispatch is instead routed through the shared execution bridge library at `${CLAUDE_PLUGIN_ROOT}/scripts/lib/execution-mode-b-bridge.sh`, which itself layers on the shared dispatch library `${CLAUDE_PLUGIN_ROOT}/scripts/lib/dispatch-teammate.sh`.

- **Spawn seam.** The analysis subagent scopes the change and drafts the quick spec. The orchestration calls `execution_spawn_subagent <persona> "gaia-quick-spec"` to obtain a persistent teammate handle. The clean-room gate in the shared library refuses any reviewer persona before a teammate is created.
- **Relay seam.** Each turn is relayed verbatim to the team lead via `execution_relay_turn <handle> <payload>`, so the generated quick spec is identical to the Mode A subagent-dispatch path — only the dispatch seam differs, never the produced output.
