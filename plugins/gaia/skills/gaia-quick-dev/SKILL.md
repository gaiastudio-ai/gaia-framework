...
- **Preserve the legacy WIP checkpoint shape (AC5, AC-EC5).** `.gaia/memory/checkpoints/quick-dev-{spec-name}.yaml` with `files_touched` entries containing `path`, `checksum: "sha256:{hex}"`, and `last_modified: ISO-8601`. The skill writes in this shape so `/gaia-resume` (Cluster 8) continues to work.
