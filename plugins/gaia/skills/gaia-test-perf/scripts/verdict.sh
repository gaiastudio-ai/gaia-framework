#!/usr/bin/env bash
# verdict.sh — gaia-test-perf Phase 4 verdict + Review Gate update (E73-S2, AC8).
#
...
#   BLOCKED          -> FAILED
#
# Default --gate is "Performance Review" (this skill targets the Performance
# Review row of the canonical six-row Review Gate seeded by init-review-gate.sh).
#
# Contract:
...
SCRIPT_NAME="gaia-test-perf/verdict.sh"
...
# pattern as gaia-test-e2e/verdict.sh.
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — verdict resolution + Review Gate update for /gaia-test-perf.
Usage:
  verdict.sh --analysis-results <path> --llm-findings <path>
              [--story-key <key>] [--gate <name>]
EOF
...
verdict="$("$RESOLVER" --skill gaia-test-perf --analysis-results "$ANALYSIS" --llm-findings "$LLM")" \
