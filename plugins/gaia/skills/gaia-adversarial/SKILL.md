...
- **Do NOT suggest downstream implementation fixes — but DO propose artifact-level refinements.** Test17 D-5 / AF-2026-06-02-6 reconciliation: a "Proposed refinement" per finding is REQUIRED by the Sage persona output contract (a concrete, narrow counter-proposal AT THE ARTIFACT LEVEL — "tighten this AC", "add a stale-fence", "name the rollback path explicitly"). It is NOT a downstream implementation fix, code patch, or sprint story. The distinction: a refinement edits THE TARGET DOCUMENT to address the cited weakness; a fix would be the downstream consumer (PM / dev / SM) taking the finding and routing it into implementation work. Both this skill's findings AND the upstream caller's response to them stay at the artifact-edit layer.
...
> `!${CLAUDE_PLUGIN_ROOT}/scripts/write-checkpoint.sh gaia-adversarial 1 target_artifact_path="$TARGET_ARTIFACT_PATH" adversarial_angle=load target_label="$TARGET_LABEL"`
...
> `!${CLAUDE_PLUGIN_ROOT}/scripts/write-checkpoint.sh gaia-adversarial 2 target_artifact_path="$TARGET_ARTIFACT_PATH" adversarial_angle=analysis target_label="$TARGET_LABEL"`
...
> `!${CLAUDE_PLUGIN_ROOT}/scripts/write-checkpoint.sh gaia-adversarial 3 target_artifact_path="$TARGET_ARTIFACT_PATH" adversarial_angle=report target_label="$TARGET_LABEL" --paths "$REPORT_PATH"`
