...
- Resolve the test-plan READ path with planning-artifacts-first precedence (E105-S2 / ADR-127 §7.2): first try `.gaia/artifacts/planning-artifacts/test-plan.md` (canonical home); then legacy `.gaia/artifacts/test-artifacts/test-plan.md` (flat) and `.gaia/artifacts/test-artifacts/strategy/test-plan.md` (E53 placement); the sharded `test-plan/index.md` form per ADR-070 is also accepted by `validate-gate.sh test_plan_exists`. Reads of test-plan throughout the skill resolve via this rule.
...
- Resolve the test-plan path via the planning-artifacts-first precedence rule from Critical Rules above (AF-2026-05-31-3 / Test14 F-19): try `.gaia/artifacts/planning-artifacts/test-plan.md` (CANONICAL home — co-located with `test-strategy.md` per E105-S2 / ADR-127 §7.2); fall back to legacy `.gaia/artifacts/test-artifacts/test-plan.md` (flat) and `.gaia/artifacts/test-artifacts/strategy/test-plan.md` (strategy/ placement) for pre-migration projects. Read the resolved file if it exists — extract planned test IDs and their categories (Unit, Integration, E2E, Manual, Performance, Security, Accessibility). **Do NOT write `.gaia/artifacts/test-artifacts/test-plan.md` as a duplicate** — Test14 F-19 caught the duplication-can-drift bug class; the canonical home is `planning-artifacts/` and the `test-artifacts/` paths are read-compat fallbacks only.
...
- Compile the traceability matrix with:
  ...
  6. Implementation-readiness gate decision (AF-2026-05-29-2 / Test09 F-23 — planning-phase-aware): the verdict has FOUR canonical values now, distinguishing healthy planning-phase runs from broken implementation runs:
