...
- Build the **Functional Requirements** matrix section:
  ...
  - **`Coverage %` formula (AF-2026-05-24-14 / Test02 F-38; AF-2026-05-29-2 / Test09 F-22).** The percentage published in the `Coverage %` column MUST be computed as `(implemented_tiers_for_this_req / required_tiers_for_this_req) × 100`, rounded to the nearest integer. `implemented_tiers_for_this_req` counts non-empty cells across the four tier columns (Unit / Integration / E2E / Manual) for this row. `required_tiers_for_this_req` is computed from BOTH the story's `risk_level` AND its `surface_type`:
