...
- Resolve the path via `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-config.sh project_config_path` (or fall back to `.gaia/config/project-config.yaml` relative to the project root).
