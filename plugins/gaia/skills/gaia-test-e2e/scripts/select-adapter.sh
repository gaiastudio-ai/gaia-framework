...
#   2. test_execution.e2e.adapter from --config <project-config-yaml>
#   3. Default: playwright-e2e
#
...
SCRIPT_NAME="gaia-test-e2e/select-adapter.sh"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — resolve e2e adapter (CLI > project-config > default).
Usage:
  select-adapter.sh [--adapter <name>] [--config <project-config-yaml>]
EOF
      exit 0 ;;
...
# 2. Project-config lookup.
  ...
  # Minimal pure-bash YAML extraction for `test_execution.e2e.adapter:`
  # without requiring yq. Looks for the nested key pattern.
  resolved="$(awk '
    /^test_execution[[:space:]]*:/ { in_te=1; next }
    in_te && /^[^[:space:]]/ { in_te=0 }
    in_te && /^[[:space:]]+e2e[[:space:]]*:/ { in_e2e=1; next }
    in_e2e && /^[[:space:]]{0,2}[^[:space:]]/ { in_e2e=0 }
    in_e2e && /^[[:space:]]+adapter[[:space:]]*:/ {
