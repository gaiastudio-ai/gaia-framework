...
- **UI-present gate (AF-2026-05-29-2 / Test09 F-19).** Before any other prereq check, read `compliance.ui_present` from `.gaia/config/project-config.yaml`. If `ui_present` is explicitly `false`, skip neutrally with the message: `"compliance.ui_present is false — this project declared no UI layer; UX design is not applicable. Run /gaia-config-compliance to update ui_present if a UI is being added."` Exit 0 (not an error). This guard mirrors `/gaia-review-a11y` and `/gaia-validate-design-a11y` which both gate on `ui_present`. Previously this skill generated UX artifacts even for headless backends (CLI / library / API-only services), wasting tokens and producing irrelevant content. Brownfield's UX-Assessor phase already gates on `has_frontend`; this rule brings the greenfield path into alignment.
- **Headless-surface sanity check (Test10 F-18).** When `ui_present:true` AND the project ships no UI artifacts (no figma block in `ux-design.md`, no design-token file `tokens.json` / `design-tokens.yaml`, and no UI source files matching `*.css`, `*.scss`, `*.jsx`, `*.tsx`, `*.vue`, `*.svelte` under the configured stack paths), emit a NOTICE-tier finding before proceeding: `NOTICE: compliance.ui_present:true but project surface looks headless — verify project-config or remove ui_present:true.` Continue the UX design flow (the user MAY be designing a UI not yet in code), but surface the NOTICE prominently so a stale `ui_present:true` does not silently produce a hollow UX doc.
