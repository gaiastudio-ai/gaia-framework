...
# Surface missing runtime dependencies at init time instead of mid-sprint-close.
# yq (mikefarah Go v4) is required by sprint-close and several config-split
# helpers; preflight the dep here so the operator can install it before
# reaching the failing skill.
