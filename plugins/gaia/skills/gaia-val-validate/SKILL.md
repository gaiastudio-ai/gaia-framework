...
- **2026-05-13 — Sentinel-Write Writer Shift.** The Val sentinel write has been relocated from the Val sub-agent context to the orchestrator's main turn. The Val persona at `plugins/gaia/agents/validator.md` §Sentinel-Write Contract now computes the sentinel content and returns it inside the returned envelope — Val MUST NOT write the sentinel file. Consumer skills write the sentinel via `plugins/gaia/scripts/lib/write-val-envelope.sh`, then assert. The writer accepts Val's returned envelope shape DIRECTLY — it unwraps a nested `sentinel_envelope` (passing `persona_sig` through verbatim) or accepts a flat top-level sentinel, so consumers pipe Val's returned envelope straight into the writer with no caller-side reshaping. Forgery resistance preserved — `persona_sig` is still computed by Val from validator.md's on-disk sha256, which the orchestrator cannot fabricate. Closes the substrate content-integrity false-fire that blocked Val-consuming skills end-to-end. Val's frontmatter `allowed-tools` is tightened: `[Read, Grep, Glob, Bash]` (no `Write`) — Val is now read-only on the filesystem under the new contract.
