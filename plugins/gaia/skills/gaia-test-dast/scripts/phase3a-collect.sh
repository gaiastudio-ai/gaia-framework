#!/usr/bin/env bash
# phase3a-collect.sh — gaia-test-dast Phase 3A evidence collection (E73-S3).
#
# Mirrors gaia-test-perf/phase3a-collect.sh:
#   1. Probes adapter availability via tool-availability-probe.sh
#   2. If `available`, invokes the adapter's run.sh to execute the DAST scan,
...
SCRIPT_NAME="gaia-test-dast/phase3a-collect.sh"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — Phase 3A evidence collection for /gaia-test-dast.
Usage:
  phase3a-collect.sh --adapter-dir <path> --output-dir <path>
                     [--story-key <key>] [--target-url <url>]
                     [--config <path>] [--timeout <seconds>]
...
case "$probe_state" in
  ...
  expected_and_missing)
    ...
    error_reason="adapter provider not on PATH ($probe_err) — install OWASP ZAP (https://www.zaproxy.org) to enable this adapter"
    ;;
  ran_and_errored)
    check_status="errored"
...
if [ "$check_status" = "passed" ]; then
  ...
  if [ "$run_rc" -ne 0 ]; then
    ...
    if [ -f "$OUTPUT_DIR/evidence/${ADAPTER_NAME}-fragment.json" ]; then
      adapter_findings_json="$(jq -c '.findings // []' "$OUTPUT_DIR/evidence/${ADAPTER_NAME}-fragment.json" 2>/dev/null || printf '[]')"
    fi
  fi
fi
...
SCHEMA_VERSION="1.0"
SKILL_NAME="gaia-test-dast"
MODEL="${DAST_MODEL:-claude-opus-4-7}"
MODEL_TEMP="${DAST_MODEL_TEMP:-0}"
...
if [ "$check_status" = "passed" ]; then
  check_obj="$(jq -nc \
    ...
    --argjson findings "$adapter_findings_json" \
    '{name: $name, scope: "project", status: $status, findings: $findings}')"
