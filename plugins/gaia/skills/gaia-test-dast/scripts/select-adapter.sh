...
#   2. test_execution.dast.adapter from --config <project-config-yaml>
#   3. Default: owasp-zap
#
...
SCRIPT_NAME="gaia-test-dast/select-adapter.sh"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — resolve DAST adapter (CLI > project-config > default).
Usage:
  select-adapter.sh [--adapter <name>] [--config <project-config-yaml>]
EOF
      exit 0 ;;
...
# 2. Project-config lookup.
  ...
  # Pure-bash YAML extraction for `test_execution.dast.adapter:` (no yq dep).
  resolved="$(awk '
    /^test_execution[[:space:]]*:/ { in_te=1; next }
    in_te && /^[^[:space:]]/ { in_te=0 }
    in_te && /^[[:space:]]+dast[[:space:]]*:/ { in_dast=1; next }
    in_dast && /^[[:space:]]{0,2}[^[:space:]]/ { in_dast=0 }
    in_dast && /^[[:space:]]+adapter[[:space:]]*:/ {
