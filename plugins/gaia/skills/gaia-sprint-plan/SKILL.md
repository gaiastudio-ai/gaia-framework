...
- **Execution-phase derivation.** Reusing the SAME `KEY|DEPS|POINTS` candidate-set file the capacity check consumes, run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/lib/phase-derivation.sh --stories-file <candidate-set>` to partition the selection into execution phases. Output is one `KEY|PHASE` line per story: phase 1 is every story with no hard dependency inside the candidate set, and a story whose hard dependencies all sit in phases <= N lands in phase N+1. Stories sharing a phase carry no ordering constraint between them — that is precisely what makes them safe to run concurrently. A dependency on a story OUTSIDE the candidate set (closed in an earlier sprint, or simply not selected) is treated as already satisfied and does not raise the phase. Soft dependencies never affect the phase — strip them from the dep cell before writing the candidate-set file, using the same dep-cell grammar the column-sourced lint applies, because the helper reads the dep column as hard dependencies only. Phases are derived HERE, before any story is written to the sprint yaml, and the derived value is carried explicitly to the commit step; nothing downstream recomputes them. The helper exits non-zero and names every member of the cycle when the selection contains a dependency cycle, emitting no partition at all — that is a hard stop: report the cycle to the user and do NOT commit the sprint.
...
- Create the sprint plan with story assignments and execution order. Group the selected stories by the execution phase derived in Step 3, render the phases in ascending order under an explicit `Phase {n}` heading, and preserve the Step 3 selection order (priority ordering) within each phase. Every phase present in the derivation gets a heading, including the case where every story is phase 1 — a single `Phase 1` group is still rendered as a group, never flattened away. Stories inside one phase carry no ordering constraint between them: that is exactly what the grouping communicates.
...
- Write `sprint-status.yaml` to `.gaia/state/sprint-status.yaml` EXCLUSIVELY via `sprint-state.sh`:
  ...
  ${CLAUDE_PLUGIN_ROOT}/scripts/sprint-state.sh inject \
    --story "{story_key}" [--phase "{phase}"] [--sprint-id "{sprint_id}"]
  ```
  Invoke `init` once (guarded on yaml absence), then `inject` once per selected story to register it in the sprint. The `init` subcommand seeds the canonical shape (`sprint_id`/`state: active`/`total_points: 0`/`goals: []`/`items: []`). The `inject` subcommand appends the story's metadata mirrored from the story file's frontmatter — the four required fields (`sprint_id`, `status`, `points`, `risk`) MUST be present in the story file before the call. `inject` is idempotent — re-running on an already-registered key is a no-op. Use `transition` only for in-sprint status changes after the entry exists. Pass each story's execution phase from Step 3 as `--phase {phase}` — a positive integer, 1-based, at most 999. The phase is derived once at selection time and is never recomputed here; `inject` records it on the story's row and omits the field entirely when the flag is absent, so a sprint planned without phases writes exactly the yaml it wrote before. Omit the flag entirely for a phase-less commit — do NOT pass an empty value, which `inject` rejects as a usage error by design (clearing an existing phase is `set-phase`'s job, not `inject`'s). If `sprint-state.sh` exits non-zero, abort cleanly and surface the error to the user. Do NOT fall back to direct YAML writes.
