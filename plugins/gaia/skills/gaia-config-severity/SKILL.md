...
description: Edit the severity section of project-config.yaml — the 5-into-3 severity map (Critical / High / Medium / Low / Info → BLOCKED / REQUEST_CHANGES / APPROVE). Section-scoped editor that preserves YAML comments and formatting. Use when "edit severity config" or /gaia-config-severity.
...
You are editing the `severity` top-level section of `project-config.yaml` — the 5-into-3 severity map that translates internal severity bands (`Critical | High | Medium | Low | Info`) to gate verdicts (`BLOCKED | REQUEST_CHANGES | APPROVE`). The downstream review system reads this map when composing gate verdicts.
...
- For per-gate severity overrides (e.g., a single review gate with stricter mappings), use `/gaia-config-gates`. The two skills are intentionally separate (one-skill-per-section).
