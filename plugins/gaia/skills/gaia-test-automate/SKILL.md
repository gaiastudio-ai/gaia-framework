...
- **Strict schema separation (AC-EC2):** `analysis-results.json` (under `.gaia/state/review/test-automate/{story_key}/`) records DETERMINISTIC ANALYSIS output (test-execution-toolkit findings) ONLY. The plan file records the GENERATIVE PLAN (what tests to write, source-file SHA-256 entries) ONLY. Zero content overlap. The plan-file path is resolved via `${CLAUDE_PLUGIN_ROOT}/scripts/lib/resolve-test-artifact-per-story.sh test-automate-plan {story_key} --write` (AF-2026-05-30-1 / Test03 §7.3 mirror layout): NEW canonical home `.gaia/artifacts/test-artifacts/epic-{epic_slug}/stories/{story_key}-{story_slug}/test-automate-plan.md`; legacy flat `.gaia/artifacts/test-artifacts/test-automate-plan-{story_key}.md` remains a read-only fallback during the migration window.
...
**Parent write — ADR-051 plan file.** Resolve the write path via `PLAN_PATH=$(bash ${CLAUDE_PLUGIN_ROOT}/scripts/lib/resolve-test-artifact-per-story.sh test-automate-plan {story_key} --write)` — this returns the new canonical per-story home `.gaia/artifacts/test-artifacts/epic-{epic_slug}/stories/{story_key}-{story_slug}/test-automate-plan.md` (AF-2026-05-30-1 / Test03 §7.3) and `mkdir -p`s the parent directory. The parent writes the structured plan-content payload atomically (per-PID temp file + `mv` rename) to `$PLAN_PATH` per ADR-051 §10.27.3. Written REGARDLESS of approval outcome (AC-EC14). The plan file frontmatter contains `plan_id`, `analyzed_sources[]` (referencing the SAME `file_hashes` from `analysis-results.json` — AC-EC6), and an empty `approval` block awaiting the Approval Gate. Legacy flat `.gaia/artifacts/test-artifacts/test-automate-plan-{story_key}.md` remains a read-only fallback during the migration window.
...
| `.gaia/artifacts/test-artifacts/epic-{epic_slug}/stories/{story_key}-{story_slug}/test-automate-plan.md` (AF-30-1; legacy flat `.gaia/artifacts/test-artifacts/test-automate-plan-{story_key}.md` accepted on read) | ADR-051 §10.27.3 | Generative plan (source SHA-256 + plan body) | Phase 6 (parent)   |
