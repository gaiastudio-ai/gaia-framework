...
if [ -n "$TEST_PATHS" ]; then
  while IFS= read -r test_path; do
    ...
    # E67-S2: implementation-aware fields. When the plan supplies a
    ...
    case "$test_path" in
      ...
      *.test.js|*.spec.js|*.test.ts|*.spec.ts)
        # Generate Jest/Vitest test file
        {
          ...
          # (E67-S2 AC4 — real assertions reference target code paths).
...
# =========================================================================
# STEP 3b — Placeholder-test-detector gate (E67-S2, FR-RSV2-1, FR-RSV2-2)
# =========================================================================
#
...
# PASSED.
#
# Refs: AC2, AC3 of E67-S2; SS 5.8 of source-report; SS 11.4 verdict
# semantics.
...
if [ -n "$STORY_ARTIFACT" ]; then
  ...
  # E67-S2 AC2: placeholder-detector hit overrides any local PASSED — the
  ...
  # E67-S2 AC3: scaffold mode NEVER flips Review Gate to PASSED — scaffold
  # output is intentional placeholder content; the row stays UNVERIFIED so
  # downstream review skills surface that automation work is incomplete.
  if [ "$SCAFFOLD_MODE" -eq 1 ]; then
    RG_VERDICT="UNVERIFIED"
  fi
