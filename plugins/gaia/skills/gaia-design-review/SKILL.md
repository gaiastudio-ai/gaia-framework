...
- **Verdict provenance guard.** Before every verdict write, call `scripts/verdict-provenance-check.sh` with the candidate verdict/notes text and the boundary-marker-wrapped project content. If the provenance check returns non-zero, refuse the verdict write. This prevents verdicts whose text is transcribed from the project content rather than independently authored.
...
2. Call `scripts/verdict-provenance-check.sh` with the candidate verdict notes and the boundary-wrapped project content from Step 1. If the check fails (non-zero exit), refuse the verdict and re-author the notes independently.
