...
#   2. If `available`, invokes the adapter's run.sh to execute the a11y scan,
...
#                      [--wcag-level <A|AA|AAA>] [--config <path>]
#                      [--timeout <seconds>]
#
# Exit codes:
...
SCRIPT_NAME="gaia-test-a11y/phase3a-collect.sh"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — Phase 3A evidence collection for /gaia-test-a11y.
Usage:
  phase3a-collect.sh --adapter-dir <path> --output-dir <path>
                     [--story-key <key>] [--target-url <url>]
                     [--wcag-level <A|AA|AAA>] [--config <path>]
...
if [ "$check_status" = "passed" ]; then
  ...
  if [ -n "$WCAG_LEVEL" ]; then
    run_args+=(--wcag-level "$WCAG_LEVEL")
  fi
  if [ -n "$TIMEOUT" ]; then
...
SCHEMA_VERSION="1.0"
SKILL_NAME="gaia-test-a11y"
MODEL="${A11Y_MODEL:-claude-opus-4-7}"
MODEL_TEMP="${A11Y_MODEL_TEMP:-0}"
