#!/usr/bin/env bash
# select-adapter.sh — resolve which a11y adapter to use (E73-S4).
#
# Precedence (first-match-wins):
#   1. --adapter <name>      — CLI override (highest priority)
#   2. test_execution.a11y.adapter from --config <project-config-yaml>
#   3. Default: axe-core-a11y
#
...
SCRIPT_NAME="gaia-test-a11y/select-adapter.sh"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
$SCRIPT_NAME — resolve a11y adapter (CLI > project-config > default).
Usage:
  select-adapter.sh [--adapter <name>] [--config <project-config-yaml>]
EOF
      exit 0 ;;
...
# 2. Project-config lookup.
  ...
  # Pure-bash YAML extraction for `test_execution.a11y.adapter:` (no yq dep).
  resolved="$(awk '
    /^test_execution[[:space:]]*:/ { in_te=1; next }
    in_te && /^[^[:space:]]/ { in_te=0 }
    in_te && /^[[:space:]]+a11y[[:space:]]*:/ { in_a11y=1; next }
    in_a11y && /^[[:space:]]{0,2}[^[:space:]]/ { in_a11y=0 }
    in_a11y && /^[[:space:]]+adapter[[:space:]]*:/ {
