...
- **Spawn seam.** The test-architect subagent (Sable) authors new test cases when a planning teammate is used; the single-prompt path remains the default. The orchestration calls `planning_spawn_subagent gaia:test-architect "gaia-edit-test-plan"` to obtain a persistent teammate handle. The clean-room gate in the shared library refuses any reviewer persona before a teammate is created.
