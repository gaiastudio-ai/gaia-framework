...
#
# Allowed write targets (.gaia/ only):
...
#   - .gaia/custom/skills/...
#
# Legacy entries removed (post-deprecation cleanup):
...
# These were dual-path during the deprecation window; a later sweep migrated all
# framework consumers to smart-fallback through .gaia/ (env > .gaia/ > legacy).
...
# Helper — emit canonical WRITE-BOUNDARY-VIOLATION halt event.
emit_halt() {
  local detail="$1"
...
echo "write-boundary.sh: REJECTED — '$path' is outside the state-free write boundary." >&2
echo "write-boundary.sh: allowed targets (.gaia/ only): .gaia/artifacts/creative-artifacts/meeting-*.md, .gaia/state/action-items.yaml, .gaia/memory/{agent}-sidecar/decisions/*.md, .gaia/memory/meeting-sessions/*.yaml, .gaia/custom/skills/*" >&2
