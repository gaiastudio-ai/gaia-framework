...
`eval` the script's stdout to populate `stack`, `agent_file`, and `sidecar_file`. Resolution order inside the script: the story's `stack:` frontmatter field wins; otherwise it falls back to `project.stack` from config, then to filesystem markers under the project root (`angular.json` → angular-dev, `package.json`/`tsconfig.json` → ts-dev, `pom.xml`/`build.gradle` → java-dev, `requirements.txt`/`pyproject.toml` → python-dev, `go.mod` → go-dev, `pubspec.yaml` → flutter-dev, `Podfile`/`AndroidManifest.xml` → mobile-dev, `sdkconfig`/`idf_component.yml`/`platformio.ini`/FreeRTOS-`CMakeLists.txt` → embedded-dev). `bash-dev` is explicit-only — it is never auto-detected from filesystem markers; set `stack: bash` in the story frontmatter or `project.stack` in config to select it. The resolved `agent_file` is the absolute path to the matching `${CLAUDE_PLUGIN_ROOT}/agents/{stack}-dev.md` developer persona.

- **Exit 0:** log `Resolved developer: {stack} (persona {agent_file})` and carry `stack` / `agent_file` forward to Steps 4–7. Derive the subagent registration name from the persona filename — `basename {agent_file} .md` (e.g. `typescript-dev`, `angular-dev`, `flutter-dev`, `java-dev`, `python-dev`, `go-dev`, `mobile-dev`, `bash-dev`, `embedded-dev`). Note the canonical `stack` token `ts-dev` maps to the `typescript-dev` persona/registration name; always dispatch by the filename-derived name, never the raw `ts-dev` token.
- **Exit 2 (unsupported stack / persona file not found):** HALT with the script's stderr. Do NOT silently fall back to a non-stack-aware orchestrator-authored implementation — the absence of a developer persona is a stop condition, not a license to self-implement. Direct the user to set the story's `stack:` frontmatter (or `project.stack` in config) to one of: ts-dev, angular-dev, flutter-dev, java-dev, python-dev, go-dev, mobile-dev, bash-dev, embedded-dev.

**Stack-developer dispatch contract (used by Steps 4–7).** Whenever this workflow dispatches the developer, it dispatches the resolved developer subagent via the **main-turn Agent tool** (`subagent_type: gaia:<persona-name>`, where `<persona-name>` is the `basename {agent_file} .md` resolved above — e.g. `gaia:typescript-dev`, `gaia:python-dev`, `gaia:bash-dev`, `gaia:embedded-dev`) with exactly ONE level of subagent nesting. The dispatch prompt MUST carry:
