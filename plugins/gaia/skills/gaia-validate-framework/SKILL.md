...
9f. **Stale-flag Registry Audit** (E86-S6, ADR-102 / FR-475 / SR-59) — invoke `plugins/gaia/scripts/check-stale-flag-registry.sh`. The script scans `_memory/` (top-level only, per ADR-102 marker contract clause 3) for files matching `.*-stale` and verifies every marker is registered in the ADR-102 registry table inside `docs/planning-artifacts/architecture/12-12-adr-detail-records.md`. Unregistered markers are CRITICAL findings — they represent a governance audit gap per SR-59. The script's stdout is folded into the validate-framework findings list at CRITICAL severity; non-zero exit propagates as a CRITICAL finding.
