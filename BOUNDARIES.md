...
> index, not a duplicate of the canonical decision records — every section
> cross-references the source of truth. Source report:
...
> The two families share the seven-phase pipeline contract but differ on side
> effects, plan-then-execute machinery, and inclusion in `/gaia-review-all`.
...
it shares Christy as owner agent and follows the same six-skill review-pipeline contract.
The review pipeline enumerates seven possible gates (the six above plus `/gaia-review-a11y`);
the canonical "six" subset listed here is the always-on review-skill set.

**Five action skills (verdict-producing, write-capable, NOT included in `/gaia-review-all`):**

| # | Skill | Phase | Owner agent |
|---|---|---|---|
...
| Plan-then-execute machinery | No | Yes |
...
`/gaia-test-run` is a manual any-env runner. They belong to the broader
"action skills" family but are out of scope for the five deployment-phase action
skills enumerated here. The mobile deployment-phase skills `/gaia-test-mobile-e2e`
and `/gaia-test-device-matrix` are mobile-platform extensions of the same
action-skill pattern; they are not counted in the canonical five but inherit the
contract.
...
> Pre-merge gates run against the source tree; deployment-phase gates run against
> a deployed environment URL or device target.
...
| **Pre-merge** | `/gaia-review-all` invoked on a story at `status: review`; runs every always-on review skill plus conditional `/gaia-review-a11y` (when `compliance.ui_present: true`) and `/gaia-review-mobile` (when `platforms[]` declared) | Per-skill owner agent (see Review Skills vs Action Skills wiring table) | Per-gate `analysis-results.json` + Review Gate row update + composite verdict |
| **Post-deploy** | Deployment-phase action skills invoked by `/gaia-deploy` orchestrator after a successful deploy: `/gaia-test-e2e`, `/gaia-test-perf`, `/gaia-test-dast`, `/gaia-test-a11y` post-deploy variant, plus mobile equivalents for mobile-platform projects | Sable (web), Talia (mobile), Soren (deploy orchestration) | Smoke-gate `analysis-results.json` + final `/gaia-deploy` verdict |

**Composite verdict on pre-merge:** Aggregator considers up to seven gates (six
always-on plus at most one conditional). First-match-wins precedence: any
`BLOCKED` → `BLOCKED`; any `REQUEST_CHANGES` → `REQUEST_CHANGES`; otherwise
...
> shell + jq pipeline; byte-identical output for identical inputs).

**Four-layer precedence, lowest to highest (later layers override earlier):**

| Order | Layer | Path pattern | Source |
|---|---|---|---|
| 1 | **Base** | `rubrics/base/{skill}.json` (six files: `code`, `qa`, `test`, `security`, `perf`, `a11y`; `mobile.json` included for mobile-platform projects) | Ships with the framework |
| 2 | **Regime** | `rubrics/regimes/{regime}.json` for each regime declared in `project-config.yaml` `compliance.regimes:`, **loaded in declaration order** | Framework (nine ship: `gdpr`, `hipaa`, `pci-dss`, `sox`, `ccpa`, `soc2`, `iso-27001`, `wcag-2.1-aa`, `wcag-2.1-aaa`; mobile platforms add: `apple-app-store`, `google-play-store`, `coppa`) |
...
skill with status `BLOCKED`. `/gaia-validate-rubric {path}` validates a single
file; `/gaia-config-validate` validates the merged result.

---

## Adapter Origins

> Shared helper: `tool-availability-probe.sh`.
...
ahead of built-ins by tool-selection precedence).
...
**Built-in adapter origins (by phase):**
...
| Phase 9 (mobile static) | SwiftLint, SwiftFormat, Detekt, ktlint, MobSF, xcsize, apkanalyzer |
| Phase 9 (mobile dynamic) | Detox, Maestro, Appium, XCUITest, Espresso |
| Phase 9 (device-farm) | Firebase Test Lab, BrowserStack, Sauce Labs |

**Discovery skills:** `/gaia-list-tools` (enumerate adapters by category),
`/gaia-tool-info {tool}` (full adapter metadata),
`/gaia-validate-rubric {path}` (validate rubric against schema).
...
|---|---|---|
| **Reviewer** | Verdict-producing, read-only fork-context agent invoked by a review skill or a deployment-phase action skill. Consumes the seven-phase pipeline. Produces `analysis-results.json` + verdict. Never mutates the source tree. | Vera (`gaia-review-qa`), Sable (`gaia-review-test`, `gaia-test-automate`, deployment-phase action skills), Zara (`gaia-review-security`), Juno (`gaia-review-perf`), Talia (`gaia-review-mobile`, mobile deployment-phase), Christy (`gaia-validate-design-a11y`), stack-specific reviewers (ts-dev, java-dev, python-dev, go-dev, flutter-dev, mobile-dev, angular-dev) for `gaia-review-code` |
| **Action** | Write-capable agent that mutates code, runs tests, or invokes external systems. Adopts the seven-phase contract with Phase 6 extended to include external write. Bound by a path allowlist and a plan-then-execute machinery contract. | Sable (test-automation actions), Soren (`gaia-deploy` orchestration) |
| **Orchestrator** | Composes reviewer/action skills, aggregates verdicts, manages gate transitions. Does not produce its own verdict; emits the composite. | `/gaia-review-all` (composite verdict aggregator), `/gaia-deploy` (Pattern A, Claude-driven, sequencing pre-deploy → deploy → post-deploy gates) |
...
For the canonical wiring table see the architecture document at
[`docs/planning-artifacts/architecture/architecture.md`](../docs/planning-artifacts/architecture/architecture.md).
...
- **Per-skill verdict vocabulary** (output of `verdict-resolver.sh`):
...
| Per-skill verdict | Review Gate row | Composite verdict | Notes |
|---|---|---|---|
...
**Composite-verdict aggregation rule (first match wins):**
...
- Three-Tier Review Pipeline (Deterministic Evidence / LLM Judgment / Scripted Verdict)
- Tool Adapter Framework (Static Pre-Merge + Dynamic Deployment-Phase + Three-State Availability Probe)
- Layered Rubric Loading (Base + Regimes + Domain + Project; RFC 7396 JSON-Merge-Patch)
- Deployment-Phase Pattern (Post-Deploy Smoke Gates + `/gaia-deploy` Pattern A)
- Mobile-as-Platform Extension (`platforms[]`, mobile stacks, store regimes, Talia overlay)
- Composite Review Verdict Gating (`/gaia-review-all` migrates from informational to gating)
...
- Decision detail records: [`docs/planning-artifacts/architecture/12-12-adr-detail-records.md`](../docs/planning-artifacts/architecture/12-12-adr-detail-records.md)
